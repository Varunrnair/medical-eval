...
export default function SemanticAnalysisPage() {
  ...
  const contributionDonut = useMemo(() => {
    ...
    const avgSbert =
      data.reduce((sum, item) => sum + (item.sbert_similarity || 0), 0) /
      data.length;
    const avgCohere =
      data.reduce((sum, item) => sum + (item.cohere_similarity || 0), 0) /
      data.length;
    const avgVoyage =
      data.reduce((sum, item) => sum + (item.voyage_similarity || 0), 0) /
      data.length;
    const avgOpenai =
      data.reduce((sum, item) => sum + (item.openai_similarity || 0), 0) /
      data.length;
    const avgBert =
      data.reduce((sum, item) => sum + (item.bert_score_f1 || 0), 0) /
      data.length;
    ...
    return {
      ...
      datasets: [
        {
          data: [avgSbert, avgCohere, avgVoyage, avgOpenai, avgBert],
          backgroundColor: [
            "#3B82F6",
            "#14B8A6",
            "#8B5CF6",
          ...
          borderColor: ["#9CA3AF", "#D1D5DB", "#A78BFA", "#99F6E4", "#FCA5A5", "#FDBA74"],
          borderWidth: 2,
        },
      ],
    };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-neutral-800 rounded-xl border border-gray-200 dark:border-neutral-700 p-6">
        <h1 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-white mb-2">
          Semantic Analysis
        </h1>
        <p className="text-gray-600 dark:text-gray-400 text-sm">
          Measuring semantic similarity between LLM responses and gold standard
          answers using multiple embedding models including SBERT, Cohere, Voyage, OpenAI, and BERTScore.
        </p>
      </div>
      ...
      {selectedData && (
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="bg-white dark:bg-neutral-800 rounded-xl border border-gray-200 dark:border-neutral-700 p-6">
              <h3 className="text-sm md:text-base font-medium text-gray-900 dark:text-gray-100 mb-3">
                Question
              </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="bg-white dark:bg-neutral-800 rounded-xl border border-gray-200 dark:border-neutral-700 p-6">
              <h3 className="text-sm md:text-base font-medium text-gray-900 dark:text-gray-100 mb-2">
                SBERT Similarity{" "}
                <span className="text-xs text-gray-500">
                  (all-mpnet-base-v2/l3cube-pune)
                </span>
              </h3>
            ...
            <div className="bg-white dark:bg-neutral-800 rounded-xl border border-gray-200 dark:border-neutral-700 p-6">
              <h3 className="text-sm md:text-base font-medium text-gray-900 dark:text-gray-100 mb-2">
                Voyage Similarity{" "}
                <span className="text-xs text-gray-500">(voyage-3.5)</span>
              </h3>
              <p className="text-lg md:text-xl font-bold text-green-600 dark:text-green-400">
            ...
            <div className="bg-white dark:bg-neutral-800 rounded-xl border border-gray-200 dark:border-neutral-700 p-6">
              <h3 className="text-sm md:text-base font-medium text-gray-900 dark:text-gray-100 mb-2">
                OpenAI Similarity{" "}
                <span className="text-xs text-gray-500">(text-embedding-3-small)</span>
              </h3>
              <p className="text-lg md:text-xl font-bold text-green-600 dark:text-green-400">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {semanticMetricsBar && (
          <ChartContainer title="Embedding Models Comparison">
            <div
        ...
        {semanticRadar && (
          <ChartContainer title="All Semantic Metrics Radar">
